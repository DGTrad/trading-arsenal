...
function renderSessionsClock(container) {
  const now = new Date();
  // Hora de Madrid (CET/CEST automático según verano/invierno)
  const madridStr = now.toLocaleString('es-ES', {timeZone: 'Europe/Madrid', hour: '2-digit', minute: '2-digit', hour12: false});
  const [madridH, madridM] = madridStr.split(':').map(n => parseInt(n));
  // Detectar offset Madrid vs UTC (1 invierno, 2 verano)
  const madridOffset = Math.round((new Date(now.toLocaleString('en-US',{timeZone:'Europe/Madrid'})) - new Date(now.toLocaleString('en-US',{timeZone:'UTC'}))) / 3600000);
  
  // Sesiones en hora Madrid (UTC + offset)
  const off = madridOffset;
  const sessions = [
    { name: 'Sídney', open: (22+off)%24, close: (7+off)%24, emoji: '🇦🇺', color: '#3498db', pairs: 'AUD/USD, NZD/USD' },
    { name: 'Tokio', open: (0+off)%24, close: (9+off)%24, emoji: '🇯🇵', color: '#e74c3c', pairs: 'USD/JPY, EUR/JPY' },
    { name: 'Londres', open: (8+off)%24, close: (17+off)%24, emoji: '🇬🇧', color: '#2ecc71', pairs: 'EUR/USD, GBP/USD' },
    { name: 'Nueva York', open: (13+off)%24, close: (22+off)%24, emoji: '🇺🇸', color: '#e67e22', pairs: 'EUR/USD, XAU/USD' }
  ];
  const utcH = madridH;
  const utcM = madridM;
    ...
    return `<div style="background:var(--bg-2);border:1px solid ${open ? s.color : 'var(--border-1)'};border-radius:10px;padding:18px;${open ? 'box-shadow:0 0 20px ' + s.color + '22;' : 'opacity:0.6;'}">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
        ...
        <div>
          ...
          <div style="font-size:11px;color:var(--text-3)">${s.open}:00 - ${s.close}:00 hora Madrid</div>
        </div>
  ...
  container.innerHTML = `
    <div style="text-align:center;margin-bottom:24px;">
      <div style="font-family:var(--font-mono);font-size:36px;font-weight:700;color:var(--gold)">${String(utcH).padStart(2,'0')}:${String(utcM).padStart(2,'0')} Madrid</div>
    ...
    <div style="margin-top:20px;padding:16px;background:var(--bg-3);border:1px solid var(--border-1);border-radius:8px;">
      <p style="font-size:13px;color:var(--text-2);line-height:1.7;">
        ...
        <strong>Londres + NY (14:00-18:00 Madrid)</strong> es la ventana con más volumen y mejores oportunidades.
