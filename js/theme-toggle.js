...
(function() {
  ...
  function toggleTheme() {
    ...
    // Si la página tiene widgets de TradingView, recargar para regenerarlos
    // con el tema correcto (los iframes no permiten cambio en caliente)
    const hasTradingViewWidgets = document.querySelector('.tradingview-widget-container') !== null;
    
    if (hasTradingViewWidgets) {
      // Aplicar tema visualmente antes de recargar (evita flash)
      if (newTheme === 'light') {
        document.documentElement.setAttribute('data-theme', 'light');
      } else {
        document.documentElement.removeAttribute('data-theme');
      }
      // Recargar tras 150ms para que se vea el cambio del botón
    ...
    // Si NO hay widgets, cambio instantáneo (como antes)
    if (newTheme === 'light') {
      document.documentElement.setAttribute('data-theme', 'light');
    } else {
      document.documentElement.removeAttribute('data-theme');
